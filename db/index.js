...
addEmployee(firstName, lastName, roles, manager) {
    return this.connect.promise().query("INSERT INTO employee (first_name, last_name, roles_id, manager_id) VALUES (?, ?,  ?, ?);", [firstName, lastName, roles, manager]);
};
...
updateRole(role, employeeId){
    return this.connect.promise().query("UPDATE employee SET roles_id = ? WHERE id = ?;", [role, employeeId])
};
};
